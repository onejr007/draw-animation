...
const startDrawing = event => {
    isMouseDown = true;   
    [x, y] = [event.offsetX || event.touches[0].clientX - this.offsetLeft, event.offsetY || event.touches[0].clientY - this.offsetTop - 40];  
}
const drawLine = event => {
    if ( isMouseDown ) {
        const newX = event.offsetX || event.touches[0].clientX - this.offsetLeft;
        const newY = event.offsetY || event.touches[0].clientY - this.offsetTop - 40;
        context.beginPath();
        context.moveTo( x, y );
